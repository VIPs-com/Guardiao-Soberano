#!/usr/bin/env bash
#
# gs-whonix-install-virtualbox.sh — Guardiao-Soberano (host Linux) - port da suite Privacy-OS-Hub whonix-host v3.5.4
#
# Passo 10 — pré-requisito: Oracle VirtualBox verificado no Debian/Ubuntu.
# Playbook: laboratorio/nivel-3-observador/01-whonix-virtualbox-completo.md
#
# Uso:
#   sudo ./gs-whonix-install-virtualbox.sh [-v VERSAO] [-y] [--no-extpack] [--skip-mok]
...
#   3 — pacote OK; falta gs-whonix-sign-virtualbox-modules.sh
#   1 — erro fatal
#
# Log: /var/log/virtualbox-install.log (linha RESULTADO: no final)
# Assinatura de módulos: gs-whonix-sign-virtualbox-modules.sh (log separado)
#
...
#   - Assinatura separada: gs-whonix-sign-virtualbox-modules.sh
...
SIGN_SCRIPT="${SCRIPT_DIR}/gs-whonix-sign-virtualbox-modules.sh"
...
invoke_sign_script() {
    ...
    if [[ ! -x "$SIGN_SCRIPT" ]]; then
        warn "gs-whonix-sign-virtualbox-modules.sh ausente em ${SIGN_SCRIPT}"
        warn "Rode: sudo ./gs-whonix-sign-virtualbox-modules.sh -y --qa-log"
        return 1
    fi
    log "Delegando assinatura → gs-whonix-sign-virtualbox-modules.sh"
    set +e
    "$SIGN_SCRIPT" -y 2>&1 | tee -a "$LOG_FILE" >&2
    sign_rc=${PIPESTATUS[0]}
    set -e
    if [[ "$sign_rc" -eq 0 ]]; then
        log "Assinatura OK (gs-whonix-sign-virtualbox-modules.sh)."
        return 0
...
print_wizard_intro() {
    ...
    case "$WIZARD_PHASE" in
        ...
        pending_mok_reboot)
            ...
            ;;
        needs_sign)
            phase_msg="Fase: ASSINAR MÓDULOS — MOK OK ou SB off; rode gs-whonix-sign-virtualbox-modules.sh."
            ;;
        installed_need_mok)
    ...
    _m ""
    _m "==================================================================="
    _m "  Assistente VirtualBox — Guardiao-Soberano · v3.5.4"
    _m "==================================================================="
    _b "  ${phase_msg}"
    echo "" >&2
    ...
    if [[ "$WIZARD_PHASE" == "pending_mok_reboot" ]]; then
        ...
        _y "  Próximo: sudo ./gs-whonix-sign-virtualbox-modules.sh -y --qa-log"
    fi
    echo "" >&2
    _b "  Validar: ./gs-whonix-verify-virtualbox-host.sh --qa-log"
...
===================================================================
  ...
  3) De volta ao Debian (ordem recomendada):
       ...
       sudo ./gs-whonix-verify-virtualbox-host.sh --qa-log
       sudo ./gs-whonix-install-virtualbox.sh -y    # Extension Pack se faltar
  ...
  · Refaça do zero:
      sudo ./gs-whonix-install-virtualbox.sh --reset-mok --new-mok-keys -y
...
handle_secure_boot_modules() {
    log "[Passo 8/11] Módulos kernel — delegado ao gs-whonix-sign-virtualbox-modules.sh"
...
print_summary() {
    local next_action="Relogin (vboxusers) → gs-whonix-import-ova.sh"
    echo "" >&2
    log "===== Resumo [11/11] ====="
    if vbox_modules_loaded; then
        log "Módulos kernel: OK (vboxdrv carregado)"
        next_action="sudo ./gs-whonix-verify-virtualbox-host.sh --qa-log → gs-whonix-import-ova.sh"
    elif [[ "$MOK_REBOOT_NEEDED" -eq 1 ]]; then
        log "Módulos kernel: PENDENTE_REBOOT_MOK"
        next_action="sudo systemctl reboot -i → Enroll MOK → gs-whonix-sign-virtualbox-modules.sh -y"
    elif virtualbox_pkg_installed; then
        log "Módulos kernel: PENDENTE_SIGN"
        next_action="sudo ./gs-whonix-sign-virtualbox-modules.sh -y --qa-log"
    else
