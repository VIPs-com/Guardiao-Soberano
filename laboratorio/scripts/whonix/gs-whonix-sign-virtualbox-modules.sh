...
#
# gs-whonix-sign-virtualbox-modules.sh — Guardiao-Soberano (host Linux) - port da suite Privacy-OS-Hub whonix-host v3.5.4
#
# Passo 10 — compilar, assinar (Secure Boot) e carregar módulos vboxdrv.
# Rode DEPOIS de gs-whonix-install-virtualbox.sh + tela azul MOK (se SB ON).
# Repita após cada apt upgrade que troca o kernel.
#
# Uso:
#   sudo ./gs-whonix-sign-virtualbox-modules.sh [-y] [--qa-log] [--sign-only]
#
# Exit codes:
...
# nativo do pacote — ver gs-whonix-install-virtualbox.sh para o mesmo fix.
...
sign_modules() {
    ...
    [[ -f "$MOK_PRIV" && -f "$MOK_DER" ]] \
        || fail "Chaves MOK ausentes em ${MOK_DIR} — rode gs-whonix-install-virtualbox.sh primeiro."
...
write_qa_log() {
    ...
    log_file="${log_dir}/10-virtualbox-sign-$(date +%Y%m%d-%H%M%S).txt"
    {
        ...
        echo "script: gs-whonix-sign-virtualbox-modules.sh"
...
main() {
    ...
    _m ""
    _m "==============================================================="
    _m "  gs-whonix-sign-virtualbox-modules.sh — assinar + carregar vbox"
    _m "==============================================================="
    _b "  Kernel: $(uname -r)"
    ...
    _b "Próximo: sudo ./gs-whonix-verify-virtualbox-host.sh --qa-log"
