...
usage() {
    grep '^#' "$0" | sed -e 's/^# \?//' -e '/^SPDX\|^Copyright/d' -e '1,/^$/d' | head -n 25
    exit 1
}
...
validate_inputs() {
    ...
    [[ -n "$KEY_FILE"      ]] || fail "Informe derivative.asc (ou chave oficial) com -k."
...
verify_signature() {
    ...
    if ! gpg_output="$(gpg --verify "$SIG_FILE" "$OVA_FILE" 2>&1)"; then
        log "$gpg_output"
        fail "Assinatura INVÁLIDA. NÃO importe este arquivo."
    fi

    if ! echo "$gpg_output" | grep -qi "Good signature"; then
        log "$gpg_output"
        fail "gpg não retornou 'Good signature'. Não prossiga."
    fi

    log "$gpg_output"
    log "Good signature confirmada."
}
