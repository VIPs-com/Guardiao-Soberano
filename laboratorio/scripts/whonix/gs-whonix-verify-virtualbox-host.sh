...
#
# gs-whonix-verify-virtualbox-host.sh — Privacy-OS-Hub
#
...
#   3 — FAIL_SIGN (MOK OK; falta gs-whonix-sign-virtualbox-modules.sh)
...
check_mok_enrolled() {
    ...
    if [[ ! -f "$MOK_DER" ]]; then
        record_check FAIL "MOK.der" "ausente — rode gs-whonix-install-virtualbox.sh"
        FAIL_MOK_DETECTED=1
        return
    fi
    local out cn
...
check_vbox_package() {
    ...
    if dpkg -l "$pkg" 2>/dev/null | grep -q '^ii'; then
        ...
        record_check FAIL "pacote ${pkg}" "não instalado — gs-whonix-install-virtualbox.sh"
    fi
}
...
check_vbox_modules() {
    ...
    if echo "$err" | grep -qi 'Key was rejected'; then
        if secure_boot_enabled && mok_key_enrolled; then
            record_check FAIL "vboxdrv" "Key was rejected — kernel novo; rode gs-whonix-sign-virtualbox-modules.sh"
        ...
        if [[ -n "$blacklist_hit" ]]; then
            ...
            if [[ -n "$vermagic_mod" && "$vermagic_mod" != "$vermagic_run" ]]; then
                detail="${detail} Provável causa: vermagic '${vermagic_mod}' != kernel rodando '${vermagic_run}' (recompile com gs-whonix-sign-virtualbox-modules.sh)."
            else
                detail="${detail} Rode: sudo dmesg | grep -i vbox"
            fi
        else
            detail="${detail} .ko não encontrado para $(uname -r) — rode gs-whonix-sign-virtualbox-modules.sh."
        fi
        record_check FAIL "vboxdrv" "$detail"
        FAIL_SIGN_DETECTED=1
    elif secure_boot_enabled && mok_key_enrolled; then
        record_check FAIL "vboxdrv" "não carregado — rode gs-whonix-sign-virtualbox-modules.sh -y"
        FAIL_SIGN_DETECTED=1
    else
...
write_qa_log() {
    ...
    fi
    {
        ...
        echo "script: gs-whonix-verify-virtualbox-host.sh"
...
main() {
    ...
    echo ""
    m "==============================================================="
    m "  gs-whonix-verify-virtualbox-host.sh — validação host (Passo 10)"
    m "==============================================================="
    ...
    if vbox_modules_loaded; then
        ...
        g "  Próximo: gs-whonix-verify-image.sh → gs-whonix-import-ova.sh"
    ...
    if [[ "$FAIL_SIGN_DETECTED" -eq 1 ]]; then
        ...
        y "  Próximo: sudo ./gs-whonix-sign-virtualbox-modules.sh -y --qa-log"
        [[ "$QA_LOG" -eq 1 ]] && write_qa_log
        exit 3
    fi
