# Nível 2 — A Carteira Fria

**Livro:** Capítulo 7 · `manuscrito/07-nivel-2-carteira-fria.md`

## Labs planejados

| Arquivo | Descrição | Status |
| --- | --- | --- |
| `01-sparrow-watching-only.md` | Import xpub + primeiro receive | 🟡 Extraído — revalidar em hardware |
| `02-primeiro-psbt.md` | Criar, assinar, transmitir (SD + QR) | 🟡 Extraído — revalidar em hardware |
| `03-keepassxc-tails.md` | Cofre `.kdbx` na persistência | 🟡 Extraído — revalidar em hardware |
| `04-electrum-tails.md` | Install + Tor + persistência | 🟡 Extraído — revalidar em hardware |
| `05-tails-setup-btc-xmr-completo.md` | Setup Tails end-to-end | 🟡 Extraído — revalidar em hardware |
