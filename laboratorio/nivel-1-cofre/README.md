# Nível 1 — O Cofre

**Livro:** Capítulo 6 · `manuscrito/06-nivel-1-cofre.md`

## Labs

| Arquivo | Descrição | Status |
| --- | --- | --- |
| `01-hw-wallet-inicializacao.md` | Setup air-gapped genérico | 🟡 Extraído — revalidar em hardware |
| `02-teste-restauracao.md` | Restaurar seed antes de fundos | 🟡 Extraído — revalidar em hardware |
| `03-psbt-via-qr.md` | Fluxo PSBT QR (xpub + assinatura) | 🟡 Extraído — revalidar em hardware |
| `04-seedsigner-vs-krux-opcional.md` | Comparativo DIY + side-by-side | 🟡 Extraído — revalidar em hardware |

## Ordem sugerida

`01` → `02` (obrigatório) → `03` → Nível 2 · `04` opcional se escolheu DIY
